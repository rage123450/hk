...
namespace els {
	...
	void Connection::start() {
		...
		*/
		...
		sendPacket(LoginPacket::serverAck(hmac_key, key).getPacket());
		m_crypto.setHMAC(hmac_key); // set new hmac
		m_crypto.setKey(key); // set new key
		m_sequence++;
		std::cout << "Connection established with " << tcpSocket.remote_endpoint() << std::endl;
...
//			std::cout << packetLen << std::endl;
	...
	void Connection::handleRead(int size, const boost::system::error_code& error) {
		...
		LoginServerHandler::handlePacket(pr, this);
